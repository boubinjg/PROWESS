...
function filterInt(value) {
  try {
    ...
    if (/^[-+]?(\d+|(\d+([e]|[E])[+]\d+))$/.test(String(value))) {
...
function filterFloat(value) {
  try {
    ...
    if (/^[-+]?(\d+|(\d+[.]\d+)|(\d+([.]\d+)*([e]|[E])[-+]\d+))$/.test(String(value))) {
...
function intValid(value) {
  try {
    ...
    return /^[-+]?(\d+|(\d+([e]|[E])[+]\d+))$/.test(String(value)); // ((/^[-+]?(\d+|(\d+[.]\d+)|(\d+([.]\d+)*([e]|[E])[-+]\d+)|Infinity)$/.test(value)));
  } catch (_unused5) {
    return false;
  }
}
...
function isRGB(value) {
  return /^[#]([a-f]|[A-F]|[0-9]){6}$/.test(String(value));
}
